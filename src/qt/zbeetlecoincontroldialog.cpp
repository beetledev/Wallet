...
ZBeetleCoinControlDialog::ZBeetleCoinControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBeetleCoinControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBeetleCoinControlDialog::~ZBeetleCoinControlDialog()
{
    delete ui;
}
...
void ZBeetleCoinControlDialog::updateLabels()
{
    ...
    ui->labelZBeetleCoin_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBeetleCoinControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBeetleCoinControlDialog::GetSelectedMints()
{
