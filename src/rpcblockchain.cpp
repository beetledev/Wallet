...
Object blockToJSON(const CBlock& block, const CBlockIndex* blockindex, bool txDetails = false)
{
    ...
    for (auto denom : libzerocoin::zerocoinDenomList) {
        zbeetlecoinObj.push_back(Pair(to_string(denom), ValueFromAmount(blockindex->mapZerocoinSupply.at(denom) * (denom*COIN))));
    }
    zbeetlecoinObj.emplace_back(Pair("total", ValueFromAmount(blockindex->GetZerocoinSupply())));
    result.emplace_back(Pair("zBEETsupply", zbeetlecoinObj));
