...
#ifndef BEETLECOIN_ZBEETWALLET_H
#define BEETLECOIN_ZBEETWALLET_H
...
public:
    CzBEETWallet(std::string strWalletFile);
    ...
    void GenerateDeterministicZBEET(libzerocoin::CoinDenomination denom, libzerocoin::PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly = false);
    ...
    void UpdateCount();
    void Lock();
    void SeedToZBEET(const uint512& seed, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key);

private:
    uint512 GetZerocoinSeed(uint32_t n);
};

#endif //BEETLECOIN_ZBEETWALLET_H
