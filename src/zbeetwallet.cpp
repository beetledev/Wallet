...
CzBEETWallet::CzBEETWallet(std::string strWalletFile)
{
    ...
    //Check for old db version of storing zbeet seed
    if (fFirstRun) {
        uint256 seed;
        if (walletdb.ReadZBEETSeed_deprecated(seed)) {
            //Update to new format, erase old
            seedMaster = seed;
            hashSeed = Hash(seed.begin(), seed.end());
            if (pwalletMain->AddDeterministicSeed(seed)) {
                if (walletdb.EraseZBEETSeed_deprecated()) {
                    LogPrintf("%s: Updated zBEET seed databasing\n", __func__);
                    fFirstRun = false;
                } else {
                    LogPrintf("%s: failed to remove old zbeet seed\n", __func__);
                }
            }
        }
    }
...
bool CzBEETWallet::SetMasterSeed(const uint256& seedMaster, bool fResetCount)
{
    ...
    if (fResetCount)
        walletdb.WriteZBEETCount(nCountLastUsed);
    else if (!walletdb.ReadZBEETCount(nCountLastUsed))
        nCountLastUsed = 0;
...
void CzBEETWallet::Lock()
{
    seedMaster = 0;
}

void CzBEETWallet::AddToMintPool(const std::pair<uint256, uint32_t>& pMint, bool fVerbose)
{
    mintPool.Add(pMint, fVerbose);
}

//Add the next 20 mints to the mint pool
void CzBEETWallet::GenerateMintPool(uint32_t nCountStart, uint32_t nCountEnd)
{
    ...
    for (uint32_t i = n; i < nStop; ++i) {
        ...
        CKey key;
        SeedToZBEET(seedZerocoin, bnValue, bnSerial, bnRandomness, key);
...
bool CzBEETWallet::LoadMintPoolFromDB()
{
...
void CzBEETWallet::RemoveMintsFromPool(const std::vector<uint256>& vPubcoinHashes)
{
    for (const uint256& hash : vPubcoinHashes)
        mintPool.Remove(hash);
}

void CzBEETWallet::GetState(int& nCount, int& nLastGenerated)
{
...
void CzBEETWallet::SyncWithChain(bool fGenerateMintPool)
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : listMints) {
            ...
            if (pwalletMain->zbeetTracker->HasPubcoinHash(pMint.first)) {
...
bool CzBEETWallet::SetMintSeen(const CBigNum& bnValue, const int& nHeight, const uint256& txid, const CoinDenomination& denom)
{
    ...
    CBigNum bnRandomness;
    CKey key;
    SeedToZBEET(seedZerocoin, bnValueGen, bnSerial, bnRandomness, key);
    ...
    // Add to zbeetTracker which also adds to database
    pwalletMain->zbeetTracker->Add(dMint, true);
...
void CzBEETWallet::SeedToZBEET(const uint512& seedZerocoin, CBigNum& bnValue, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
uint512 CzBEETWallet::GetZerocoinSeed(uint32_t n)
{
...
void CzBEETWallet::GenerateDeterministicZBEET(CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint, bool fGenerateOnly)
{
...
void CzBEETWallet::GenerateMint(const uint32_t& nCount, const CoinDenomination denom, PrivateCoin& coin, CDeterministicMint& dMint)
{
...
bool CzBEETWallet::RegenerateMint(const CDeterministicMint& dMint, CZerocoinMint& mint)
{
